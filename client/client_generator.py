...
class ClientGenerator:
    ...
    def send_request(self, server_url: str, server_name: str, req: JobRequest) -> JobResult:
        ...
        try:
            ...
            response = requests.post(
                f"{server_url}/solve",
                json=payload,
                timeout=300  # 5 min timeout for large matrices
            )
    ...
    def print_summary(self):
        ...
        for server, server_results in self.results.items():
            ...
            failed = [r for r in server_results if r.status != 'completed']
        ...
        if 'python' in self.results and 'cpp' in self.results:
            py_completed = [r for r in self.results['python'] if r.status == 'completed']
            cpp_completed = [r for r in self.results['cpp'] if r.status == 'completed']
            
            if py_completed and cpp_completed:
                py_avg = sum(r.solver_time_ms for r in py_completed) / len(py_completed)
                cpp_avg = sum(r.solver_time_ms for r in cpp_completed) / len(cpp_completed)
