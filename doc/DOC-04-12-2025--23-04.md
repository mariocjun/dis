...
int main(int argc, char *argv[]) {
    ...
    for (const auto &config: tests) {
        ...
        try {
            auto [std_metrics, precond_metrics] = run_sparse_comparison(config);
            all_results.push_back({config.description, std_metrics, precond_metrics});
