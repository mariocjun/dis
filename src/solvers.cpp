...
ReconstructionResult run_cgnr_solver_epsilon_save_iters(
  ...
  if (H_model.rows() != g_signal.size())
    throw std::runtime_error("run_cgnr_solver_epsilon_save_iters: Dimensoes "
                             "H/g incompativeis. H.rows=" +
                             std::to_string(H_model.rows()) +
                             ", g.size=" + std::to_string(g_signal.size()));
  if (H_model.cols() <= 0)
    throw std::runtime_error(
        "run_cgnr_solver_epsilon_save_iters: Matriz H tem " +
        std::to_string(H_model.cols()) + " colunas.");
  if (H_model.rows() == 0) {
    std::cerr << "[AVISO] run_cgnr_solver_epsilon_save_iters: Matriz H ou "
                 "sinal g estao vazios."
              << std::endl;
  ...
  bool save_iters =
      !base_filename_prefix.empty() && img_rows > 0 && img_cols > 0;
  if (save_iters) {
    try {
      std::filesystem::path iter_img_path =
          output_dir / (base_filename_prefix + "_iter_0.csv");
  ...
  for (int i = 0; i < max_iterations; ++i) {
    ...
    if (modified_denominator < std::numeric_limits<double>::epsilon()) {
      std::cout << "[INFO] Denominador modificado (" << modified_denominator
                << ") proximo de zero na iteracao " << i + 1 << ". Parando."
                << std::endl;
      break;
    }
    ...
    // --- DIAGNOSTICS PHASE 2: CONVERGENCE & ARTIFACTS (Standard) ---
    double f_max = f.maxCoeff();
    double f_min = f.minCoeff();
    double f_mean = f.mean();
    double f_sq_sum = (f.array() - f_mean).square().sum();
    double f_std = std::sqrt(f_sq_sum / f.size());

    // Artifact Detection via CV
    double amplitude = std::max(std::abs(f_max), std::abs(f_min));
    double cv_f = (amplitude > 1e-12) ? (f_std / amplitude) : 0.0;

    std::string artifact_status = "LIMPO";
    if (cv_f > 0.8)
      artifact_status = "ALTO_RUIDO";
    else if (cv_f > 0.5)
      artifact_status = "SUSPEITOSO";

    double taxa_reducao = (previous_residual_norm > 1e-12)
                              ? current_residual_norm / previous_residual_norm
                              : 1.0;

    if (i > 0 && current_residual_norm > previous_residual_norm * 1.05) {
      std::cout << "[WARN] DIVERGENCIA DETECTADA na iteracao " << i + 1
                << ". Residuo aumentou de " << previous_residual_norm
                << " para " << current_residual_norm << std::endl;
    }

    std::cout << "[DEBUG Loop] Iter " << i + 1 << " | Res: " << std::scientific
              << std::setprecision(4) << current_residual_norm
              << " | Eps: " << epsilon << " | Rate: " << std::fixed
              << std::setprecision(3) << taxa_reducao << " | CV: " << cv_f
              << " (" << artifact_status << ")" << std::defaultfloat
              << std::endl;
    // ----------------------------------------------------
    ...
    if (z_norm_sq >= std::numeric_limits<double>::epsilon()) {
      ...
      std::cout << "[INFO] ||z||^2 (" << z_norm_sq
                << ") proximo de zero na iteracao " << i + 1
...
// --- Implementação do Solver CGNR Pré-condicionado (Salva Imagens
// Intermediárias) ---
template <typename MatrixType>
inline ReconstructionResult run_cgnr_solver_preconditioned_save_iters(
  ...
  // Measure Preconditioner Setup Time
...
#pragma omp parallel for schedule(static)
  ...
  double time_precond_ms =
      std::chrono::duration<double, std::milli>(end_precond - start_precond)
          .count();

  std::cout << "[INFO] Pre-condicionador Jacobi calculado in " << std::fixed
            << std::setprecision(2) << time_precond_ms << " ms"
            << std::defaultfloat << std::endl;

  Eigen::VectorXd z =
      z_unprec.cwiseProduct(preconditioner); // z_0 = M^-1 * (H^T * r_0)
  ...
  for (int i = 0; i < max_iterations; ++i) {
    ...
    double alpha = z_precond_dot_z /
                   modified_denominator; // Usa produto interno modificado
    ...
    Eigen::VectorXd z_next =
        z_next_unprec.cwiseProduct(preconditioner); // Aplica pré-condicionador
    double z_next_precond_dot_z_next =
        z_next_unprec.dot(z_next); // Novo produto interno
    ...
    // --- DIAGNOSTICS PHASE 2: CONVERGENCE & ARTIFACTS (Precond) ---
    double f_max = f.maxCoeff();
    double f_min = f.minCoeff();
    double f_mean = f.mean();
    double f_sq_sum = (f.array() - f_mean).square().sum();
    ...
    if (z_precond_dot_z >= std::numeric_limits<double>::epsilon()) {
      ...
      std::cout << "[INFO] Produto interno z^T M^-1 z_unprec ("
                << z_precond_dot_z << ") proximo de zero na iteracao " << i + 1
                << ". Usando beta=0 (restart)." << std::endl;
      if (z_next_precond_dot_z_next < std::numeric_limits<double>::epsilon()) {
        std::cout << "[INFO] Produto interno z_next^T M^-1 z_unprec_next "
                     "tambem proximo de zero. Provavelmente estagnou. Parando."
                  << std::endl;
        break;
      }
    }
  ...
  result.final_error = current_residual_norm;
  result.final_epsilon = epsilon;
  result.precond_setup_time_ms = time_precond_ms;
  result.execution_time_ms =
...
run_cgnr_solver_fixed_iter(const Eigen::VectorXd &g_signal,
  ...
  if (H_model.rows() != g_signal.size())
    throw std::runtime_error(
        "run_cgnr_solver_fixed_iter: Dimensoes H/g incompativeis");
  if (H_model.cols() <= 0)
    throw std::runtime_error(
        "run_cgnr_solver_fixed_iter: Matriz H tem 0 colunas");
  if (H_model.rows() == 0)
    return ReconstructionResult{};

  const auto start_time = std::chrono::high_resolution_clock::now();
  Eigen::VectorXd f = Eigen::VectorXd::Zero(H_model.cols());
  Eigen::VectorXd r = g_signal; // r_0 = g - H*0 = g
  Eigen::VectorXd z = H_model.transpose() * r;
  Eigen::VectorXd p = z;
  double z_norm_sq = z.squaredNorm();
  ...
  for (int i = 0; i < num_iterations; ++i) {
    ...
    Eigen::VectorXd z_new = H_model.transpose() * r;
    double z_new_norm_sq = z_new.squaredNorm();
    double beta = 0.0;
    if (z_norm_sq >= std::numeric_limits<double>::epsilon()) {
    ...
    p = z_new + beta * p;
    z = z_new;
    z_norm_sq = z_new_norm_sq;
  }

  const auto end_time = std::chrono::high_resolution_clock::now();
  result.image = f;
  result.final_error = r.norm(); // Last residual
  result.execution_time_ms =
      std::chrono::duration<double, std::milli>(end_time - start_time).count();
  return result;
}
...
ReconstructionResult run_fista_solver_save_iters(
  ...
  for (int iter = 0; iter < max_iterations; ++iter) {
    ...
    x = (x.array().abs() > lambda)
            .select((x.array().abs() - lambda) * x.array().sign(), 0.0);
    ...
    if (img_rows > 0 && img_cols > 0) {
      try {
        std::filesystem::path iter_img_path =
            output_dir /
            (base_filename_prefix + "_iter_" + std::to_string(iter) + ".csv");
        saveImageVectorToCsv(x, iter_img_path.string(), img_rows, img_cols);
      } catch (const std::exception &e) {
        std::cerr << "[AVISO] Falha ao salvar imagem iter " << iter
                  << " (FISTA): " << e.what() << std::endl;
      }
    }

    // Check convergence
    if (iter > 0) {
      double epsilon = std::abs(result.residual_history[iter] -
                                result.residual_history[iter - 1]);
      result.final_epsilon = epsilon;
      if (epsilon < tolerance) {
        result.converged = true;
        break;
...
/* // Não estamos usando pré-condicionador denso, então esta pode ficar
comentada se quiser template ReconstructionResult
run_cgnr_solver_preconditioned_save_iters<Eigen::MatrixXd>( const
Eigen::VectorXd &g_signal, const Eigen::MatrixXd &H_model, double tolerance,
int max_iterations, const std::string& base_filename_prefix, const
std::filesystem::path& output_dir, int img_rows, int img_cols);
*/
...
/*
template ReconstructionResult
run_cgnr_solver_epsilon_save_iters<Eigen::MatrixXd>( const Eigen::VectorXd
&g_signal, const Eigen::MatrixXd &H_model, double tolerance, int
max_iterations, const std::string& base_filename_prefix, const
std::filesystem::path& output_dir, int img_rows, int img_cols);
...
template ReconstructionResult
run_fista_solver_save_iters<Eigen::SparseMatrix<double>>(
    const Eigen::VectorXd &g_signal, const Eigen::SparseMatrix<double> &H_model,
    double tolerance, int max_iterations,
    const std::string &base_filename_prefix,
    const std::filesystem::path &output_dir, int img_rows, int img_cols);
...
template ReconstructionResult run_fista_solver_save_iters<Eigen::MatrixXd>(
    const Eigen::VectorXd &g_signal, const Eigen::MatrixXd &H_model,
    double tolerance, int max_iterations,
    const std::string& base_filename_prefix, const std::filesystem::path&
output_dir, int img_rows, int img_cols);
*/
